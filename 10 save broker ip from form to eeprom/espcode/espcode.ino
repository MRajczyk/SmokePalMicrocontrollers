...
String responseHTML = ""
                      ...
                      "<form action=\"/save\" method=\"POST\">"
                        ...
                        "<input type=\"text\" name=\"brokerip\" placeholder=\"brokerip\" required><br />"
...
char eeprom_broker_ip[STR_LEN] = {0};
...
void handle_form() {
  ...
  if (webServer.hasArg("brokerip")) {
    broker_ip = webServer.arg("brokerip");
  }
  ...
  broker_ip.toCharArray(eeprom_broker_ip, STR_LEN);

  EEPROM.put(0, eeprom_ssid);
  EEPROM.put(128, eeprom_password);
  EEPROM.put(256, eeprom_broker_ip);
  EEPROM.commit();
  EEPROM.end();
  delay(1000);
...
void setup() {
  ...
  EEPROM.get(256, eeprom_broker_ip);
  mqtt_server.fromString(String(eeprom_broker_ip));
